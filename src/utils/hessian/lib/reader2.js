var assert = require('assert'),
    BufferReader = require('./buffer-reader-zh'),
    Long = require('long'),
...
Reader.prototype.readString = function(data) {
    ...
    var code = this.reader.nextUInt8();
    if (code >= 0 && code < 32) {
        return this.reader.nextString2(code);
    } else if (code >= 0x30 && code <= 0x33) {
        this.reader.move(-1);
        var len = this.reader.nextUInt16BE() - 0x3000;
        return this.reader.nextString2(len);
    } else if (code === 0x53) {
        var len = this.reader.nextUInt16BE();
        return this.reader.nextString2(len);
    } else if (code === 0x52) {
        var len = this.reader.nextUInt16BE();
        return this.reader.nextString2(len) + this.readString();
    }
};
